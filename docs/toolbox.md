# 工具箱

> 這一頁放四件事：可以直接勾選的 checklist、求職進度追蹤表範本、可以套用的 AI 提示詞範本、以及本站所有內容的原始來源清單。

---

## 一、checklist：投遞前自我檢查

勾選狀態只存在你自己的瀏覽器裡，換裝置或清除網站資料就會消失，重要的進度請自行另外記錄。

### 1.1 履歷內容自檢

- [ ] 這份履歷有依目標職缺調整內容，不是同一份履歷投所有職缺？
- [ ] 每一條經歷都寫成了「成果」，而不是「職責清單」？
- [ ] 至少大部分條列套用了 APR 句型（行動＋專案或問題＋結果）？
- [ ] 沒有可信數字的條列，改用可驗證的範圍描述，而不是硬湊百分比？
- [ ] 每個強動詞後面都接了對象、方法與結果？
- [ ] 沒有拼字、日期、聯絡方式的錯誤？
- [ ] 沒有誇大職稱、年資或技術熟練度？
- [ ] 沒有把團隊成果全部寫成個人成果？
- [ ] 沒有洩漏客戶名稱、弱點細節、內部架構或前公司機密？
- [ ] 版面是單欄、標準字型、無圖表，貼進純文字編輯器檢查過順序沒有錯亂？
- [ ] 履歷、LinkedIn、104、Cake 上的日期、職稱、成果彼此一致？
- [ ] 用 AI 工具協助草擬或改寫過的段落，已經自己重讀並修掉罐頭式措辭？

### 1.2 LinkedIn 個人檔案自檢

- [ ] Headline 清楚陳述目前或目標角色、點出一到兩個核心技能、暗示能帶來的價值？
- [ ] About 開頭兩三句話就有具體重點，不是模糊的自我介紹？
- [ ] 目標職缺的關鍵字同時出現在 Headline、About、Experience？
- [ ] Experience 裡的每一筆都是真實的任職關係，不是把客戶或使用者關係寫成工作經歷？
- [ ] Skills 欄位依目標職務優先順序排列，而不是塞滿上限？
- [ ] Featured 區塊放了具體的作品、簡報或專案連結？

### 1.3 104／Cake 自檢

- [ ] 「希望職稱／職類」聚焦相近方向，沒有寫「不拘」或混列不相關職類？
- [ ] 重要專案成就直接寫進對應欄位，不是只放在附件裡？
- [ ] 自傳依「背景定位 → 專業能力與成果 → 應徵動機 → 未來方向」的順序客製，不是從家庭背景寫起？
- [ ] Cake 的作品集或履歷精選內容跟目標求職方向一致，不會讓招募者看不出主要方向？
- [ ] 如果 Cake 履歷還要投外部 ATS，已經另外準備一份單欄無圖片的簡式版本？

### 1.4 面試準備自檢

- [ ] 針對目標職缺，整理過「能力、題型、證據」矩陣？
- [ ] 準備了六到八則涵蓋不同能力面向的 STAR 故事？
- [ ] 每則故事的 Result 段落都有可衡量的佐證，不是空泛的「進展順利」？
- [ ] 針對目標職能（軟體工程／AI 工程／其他）做了對應的技術面試準備？
- [ ] 想好了薪資區間的下限與話術，不是等被問到才臨場想？
- [ ] 準備了至少五題反問問題，聚焦具體事件而不是空泛的「文化如何」？

---

## 二、求職進度追蹤表

投遞的職缺一多，很容易忘記哪家公司該回覆了、哪一場面試還沒寄感謝信。把下面這份表格複製到自己的筆記或試算表，每投一個職缺就新增一列：

| 職缺 | 投遞日期 | 階段 | 後續行動 |
|---|---|---|---|
| A 公司／資料分析師 | 2026-09-01 | 已投遞 | 09-15 前沒有回音，寄信詢問進度 |
| B 公司／後端工程師 | 2026-09-03 | 一面 | 當天寄感謝信，整理被追問時答不好的題目 |
| | | | |

「階段」固定用幾個詞，例如已投遞、書面審查、一面、二面、錄取通知、未錄取，之後比較好篩選。「後續行動」寫具體動作和日期，不要只寫「等通知」。這份表會記錄真實的公司與進度，請存在自己的裝置或私人檔案裡，不要放進公開的 repo。

---

## 三、AI 提示詞範本

!!! warning "使用前必讀"
    這些範本的目的是提高提問的規格化程度，不是把判斷外包出去。每一段 AI 產出都必須經過你自己的驗證與潤飾，不能直接拿去投遞。貼入任何素材前，先移除真實姓名、公司內部代號、客戶名稱等可回推個人或雇主的資訊。範本中的 `⟨⟩` 都要換成你自己的內容。

### 3.1 履歷條列改寫

```
【情境與輸入】
我要應徵⟨目標職缺⟩，以下是我的一段原始經歷描述：
「⟨貼上你原本的職責描述，愈具體愈好⟩」

【預期產出】
請幫我把這段話改寫成 2-3 個版本的履歷條列，
每一版都套用「行動 + 專案或問題 + 結果」的句型，
盡量凸顯量化的規模、時間或影響；如果我提供的內容裡沒有具體數字，
請明確告訴我還缺少什麼資訊，不要自己編造數字。

【邊界與作業原則】
1. 不要生成任何我沒有提供的具體數字或成果。
2. 不要使用「負責」「協助」這類空泛動詞當開頭。
3. 改寫後請附一句話說明這個版本強調的是哪個面向（技術深度／商業影響／領導力等）。
```

### 3.2 STAR 故事草稿

```
【情境與輸入】
我要準備⟨目標職缺⟩的行為面試，這是我想講的一段經歷：
「⟨貼上你記得的事件經過，包含背景、你做了什麼、後來怎麼樣⟩」

這則故事我希望對應的能力面向是：⟨協作／衝突／成果導向／顧客溝通／判斷取捨／適應學習／失敗反思／主動性，挑一到兩個⟩

【預期產出】
請幫我整理成 STAR 架構的草稿：
- Situation、Task 各 1-2 句話
- Action 是重點，請幫我列出 3-5 個可能被追問的具體判斷點
- Result 請標出哪裡可以放數字或可驗證的結果，哪裡我需要自己補充
- 最後加一句 Reflection：學到什麼、下次會怎麼做不同

【邊界與作業原則】
1. 不要幫我編造我沒有提到的細節或數字。
2. 用「我」而非「我們」描述行動段落。
3. 如果這段經歷內容不足以撐起完整的 STAR，請直接告訴我哪裡資訊不夠，不要腦補。
```

### 3.3 模擬面試官追問

```
【角色】
請扮演一位⟨目標職缺，例如「AI 工程師」⟩的資深面試官，
風格直接、會針對模糊或空泛的回答持續追問，但不刁難、不人身攻擊。

【情境與輸入】
我準備了以下 STAR 故事，請你先聽我完整講一次：
「⟨貼上你的 STAR 故事⟩」

【預期產出】
1. 針對我這則故事，提出 3 個你認為最可能被追問的細節問題。
2. 如果我的 Result 段落沒有具體數字或可驗證的結果，指出這一點。
3. 給我一句話的整體回饋：這則故事目前最大的弱點是什麼。

【邊界與作業原則】
1. 不要幫我編造回答內容，追問完就停下來等我自己回答。
2. 回饋要具體指出問題所在，不要只給「很好」「加油」這類空泛鼓勵。
```

### 3.4 跨領域背景敘事草稿

```
【情境與輸入】
我的背景是⟨原本的領域或職務⟩，現在想轉往⟨目標領域或職務⟩。
我目前的具體經驗包括：⟨列出 3-5 項具體專案或工作內容⟩。

【預期產出】
請幫我草擬一段 2-3 句話的定位敘事，說明：
1. 原本背景給了我什麼在目標領域用得上的能力
2. 這個轉換是刻意的一步，不是不得已的備案
3. 我目前還在補強什麼，避免我誇大自己已經具備的能力

【邊界與作業原則】
1. 不要把我原本的經歷直接改名成目標領域的經驗（例如不要把「軟體開發經驗」直接寫成「AI 工程師年資」）。
2. 我如果尚未實際做過某件事，請幫我用「正在建立／正在學習」這類措辭，不要寫成已經主導完成。
3. 草稿完成後，幫我列出這段敘事裡有哪些主張是我需要準備具體證據支撐的。
```

---

## 四、延伸閱讀與資源指南

以下是本站兩份研究筆記（Claude 版、Codex 版）引用過的所有來源，依主題分類並去除重複連結。**這份清單只是入口，不是本站內容的引用依據**，實際查證請自行點開原始連結確認當下版本，平台介面與規則變動頻繁。

!!! note "查證時間"
    以下連結查證時間為 2026 年 9 月上旬，平台功能、欄位上限與市場慣例可能已經更新，投遞或引用前請自行到原始頁面確認。

### 4.1 大學職涯中心與官方履歷指南

| 來源 | 連結 |
|---|---|
| UC Berkeley Career Engagement — Resumes | <https://www.career.berkeley.edu/prepare-for-success/resumes/> |
| UC Berkeley — Evaluate Offer | <https://www.career.berkeley.edu/find-opportunities/negotiating-offers/evaluate-offer/> |
| Harvard College — Guide to Creating a Strong Resume | <https://careerservices.fas.harvard.edu/resources/create-a-strong-resume/> |
| Harvard Extension School — Create Impactful Resumes and Cover Letters | <https://careerservices.fas.harvard.edu/resources/hes-create-impactful-resumes-and-cover-letters/> |
| Harvard — Resume Bullet Points（PDF，2024） | <https://cdn-careerservices.fas.harvard.edu/wp-content/uploads/sites/161/2024/07/resume-bullets.pdf> |
| Harvard Extension School — Resumes & Cover Letters（PDF，2024） | <https://cdn-careerservices.fas.harvard.edu/wp-content/uploads/sites/161/2024/10/2024-HES_resume-and-letter.pdf> |
| Yale OCS — Writing Impactful Resume Bullets | <https://ocs.yale.edu/resources/writing-impactful-resume-bullets/> |
| Yale OCS — Resume Action Verbs | <https://ocs.yale.edu/resume-action-verbs/> |
| Yale OCS — Resume Formatting and Common Errors | <https://ocs.yale.edu/resources/resume-formatting/> |
| Yale OCS — Job Offers & Salary Negotiations | <https://ocs.yale.edu/channels/job-offers-salary-negotiations/> |
| University of Illinois (UIUC) — Action Words | <https://www.careercenter.illinois.edu/action-words> |
| University of Illinois (UIUC) — How to Write a Resume | <https://www.careercenter.illinois.edu/howtoresume> |
| University of Michigan — Interviewing Resources | <https://careercenter.umich.edu/content/interviewing-resources> |
| University of Virginia — Interviews and Offers | <https://career.virginia.edu/channels/interview/> |
| University of Virginia — STAR Method | <https://career.virginia.edu/resources/star-method/> |
| University of Pennsylvania Career Services — Write a Resume/CV | <https://careerservices.upenn.edu/channels/resume/> |
| University of Pennsylvania — Negotiating Salary and Evaluating an Offer（2019） | <https://careerservices.upenn.edu/blog/2019/08/19/negotiating-salary/> |
| MIT CAPD — Using the STAR Method | <https://capd.mit.edu/resources/the-star-method-for-behavioral-interviews/> |
| San José State University — Behavioral Interview Questions | <https://ischool.sjsu.edu/behavioral-interview-questions> |

### 4.2 求職平台官方說明

| 來源 | 連結 |
|---|---|
| LinkedIn Help — Create a good LinkedIn profile | <https://www.linkedin.com/help/linkedin/answer/a554351/how-do-i-create-a-good-linkedin-profile-?lang=en> |
| LinkedIn Help — Your LinkedIn profile overview | <https://www.linkedin.com/help/linkedin/answer/a564064/your-linkedin-profile-overview?lang=en> |
| LinkedIn — 10 Profile Headlines to Inspire Your Own（2025） | <https://www.linkedin.com/business/talent/blog/product-tips/recruiters-with-eye-catching-linkedin-profile-headlines> |
| LinkedIn Help — Editing the About section | <https://www.linkedin.com/help/linkedin/answer/a553140/editing-the-about-section-in-your-profile?lang=en> |
| LinkedIn — 14 Profile Summary Examples | <https://www.linkedin.com/business/talent/blog/product-tips/linkedin-profile-summaries-that-we-love-and-how-to-boost-your-own> |
| LinkedIn Help — Manage your Experience section | <https://www.linkedin.com/help/linkedin/answer/a593695/manage-your-experience-section?lang=en> |
| LinkedIn Help — Display Order of Skills | <https://www.linkedin.com/help/linkedin/answer/a568137/display-order-of-skills?lang=en> |
| LinkedIn — 12 steps to a better profile（2025） | <https://www.linkedin.com/business/sales/blog/profile-best-practices/17-steps-to-a-better-linkedin-profile-in-2017> |
| LinkedIn Help — Feature samples of your work | <https://www.linkedin.com/help/linkedin/answer/a550399/feature-samples-of-your-work-on-your-linkedin-profile> |
| LinkedIn Help — Featured section FAQs | <https://www.linkedin.com/help/linkedin/answer/a552452/featured-section-on-your-profile-faqs> |
| 104 — 寫履歷，到底自產好還是改寫範本？ | <https://profile.104.com.tw/post/cms_article_906> |
| 104 — 破解履歷演算法！8 個關鍵區 | <https://profile.104.com.tw/post/cms_article_23581> |
| 104 FAQ — 履歷刊登／修改 | <https://www.104.com.tw/faq/resume> |
| 104 FAQ — 主動應徵 | <https://www.104.com.tw/faq/apply> |
| 104 — 如何撰寫履歷自傳？3 段步驟（2020） | <https://blog.104.com.tw/how-to-write-a-resume-autobiography-3-steps-suggest-for-you/> |
| 104 — 還在煩惱自傳怎麼寫？ | <https://profile.104.com.tw/post/cms_article_7829> |
| 104職場力 — 新鮮人履歷技巧（2020） | <https://blog.104.com.tw/2020-new-freshman-resume-template/> |
| 104職場力 — 談薪水9招實戰技巧 | <https://blog.104.com.tw/salary-talk-guide/> |
| 104職場力 — 人資教你薪資談判6大重點 | <https://blog.104.com.tw/how-to-negotiate-salary/> |
| Cake Help — How to Create a New Resume? | <https://help.cake.me/en/articles/11532127-how-to-create-a-new-resume> |
| Cake Help — How to Create a New Resume?（舊版說明，2024） | <https://help.cake.me/en/support/solutions/articles/60000355185-how-to-create-a-new-resume-> |
| Cake — 人資愛看的履歷表怎麼寫？ | <https://www.cake.me/resources/resume/resume-outline-and-samples?locale=zh-TW> |
| Cake — 英文履歷 5 大技巧 | <https://www.cake.me/resources/6-paragraphs-of-a-perfect-english-resume?locale=en> |
| Cake — 怎麼無痛轉職 | <https://www.cake.me/resources/changing-career-path-resume-interview-and-salary> |
| Cake — Build a Free Online Portfolio | <https://www.cake.me/online-portfolio> |
| Cake — 作品集資源與教學 | <https://www.cake.me/resources/portfolio?locale=zh-TW> |
| Cake Help — How to Link My Portfolio to Resume?（2026） | <https://help.cake.me/en/articles/11532189-how-to-link-my-portfolio-to-resume-on-cake> |
| Cake Help — How to Feature Resumes or Portfolios?（2026） | <https://help.cake.me/en/articles/11532155-how-to-feature-resumes-or-portfolios-on-your-profile> |

### 4.3 履歷寫作與 ATS：業界媒體與部落格

| 來源 | 連結 |
|---|---|
| Meritamerica — 15 Must-Have Resume Tips for 2026 | <https://meritamerica.org/blog/resume-tips-2026-free-templates-ats-checklist/> |
| WAHResume — Master Quantified Achievements: Action Verbs & Metrics 2026 | <https://www.wahresume.com/blog/master-quantified-achievements-action-verbs-metrics-2026> |
| InterviewPanel — 340+ Resume Action Verbs & Power Words for 2026 | <https://www.interviewpanel.org/blog/340-resume-action-verbs-power-words-for-2026/> |
| The Interview Guys — Resume Red Flags: 12 Innocent Mistakes | <https://blog.theinterviewguys.com/resume-red-flags/> |
| Teal — 13 Resume Red Flags | <https://www.tealhq.com/post/resume-red-flags> |
| NG Career Strategy — Avoid These 30 Resume Red Flags to Get Hired in 2026 | <https://www.ngcareerstrategy.com/avoid-these-30-resume-red-flags-to-get-hired-in-2026/> |
| Enhancv — Resume Length: How Long Should a Resume Be in 2026 | <https://enhancv.com/blog/how-long-should-a-resume-be/> |
| Monster — One Page or Two-Page Resume | <https://www.monster.com/career-advice/resume/one-page-vs-two-page-resume> |
| Indeed — How Long Should a Resume Be?（2025） | <https://www.indeed.com/career-advice/resumes-cover-letters/how-long-should-a-resume-be> |
| Indeed — Why Action Verbs Are Important for Resumes（2026） | <https://www.indeed.com/career-advice/resumes-cover-letters/action-verbs> |
| Indeed — ATS-Friendly Resume: 18 Tips | <https://www.indeed.com/career-advice/resumes-cover-letters/automated-screening-resume> |
| Indeed — How to Write an ATS Resume（2026） | <https://www.indeed.com/career-advice/resumes-cover-letters/ats-resume-template> |
| Jobscan — Why ATS Tables and Columns Break Your Resume Parsing | <https://www.jobscan.co/blog/resume-tables-columns-ats/> |
| ResumeAdapter — ATS Resume Formatting Rules 2026 | <https://www.resumeadapter.com/blog/ats-resume-formatting-rules-2026> |
| Scale.jobs — How to Optimize Your Resume for ATS in 2026 | <https://scale.jobs/blog/optimize-resume-for-ats-2026-guide> |
| IPIB Resume — 10 Key Differences Between LinkedIn and Resume | <https://www.ipibresume.com/blog/blog/7-key-difference-between-linkedin-and-resume/> |
| Threads — LinkedIn vs 104 差異討論 | <https://www.threads.com/@mindreadatwork/post/DHJZ5fOPKFg/> |
| Skrapp — LinkedIn Profile Optimization Tips for 2026 | <https://skrapp.io/blog/linkedin-profile-optimization/> |

### 4.4 面試準備、企業招募頁與職業框架

| 來源 | 連結 |
|---|---|
| Microsoft Careers — Interview tips for all roles | <https://careers.microsoft.com/v2/global/en/hiring-tips/interview-tips.html> |
| Microsoft Careers — Technical interviews | <https://careers.microsoft.com/v2/global/en/hiring-tips/technical-interviewing.html> |
| BCG Careers — Case Interview Preparation | <https://careers.bcg.com/global/en/case-interview-preparation> |
| U.S. Bureau of Labor Statistics — Computer Systems Analysts | <https://www.bls.gov/ooh/computer-and-information-technology/computer-systems-analysts.htm> |
| U.S. Bureau of Labor Statistics — Information Security Analysts | <https://www.bls.gov/ooh/computer-and-information-technology/information-security-analysts.htm> |
| Indeed — How To Use the STAR Interview Response Technique | <https://www.indeed.com/career-advice/interviewing/how-to-use-the-star-interview-response-technique> |
| The Interview Guys — The STAR Method: Complete Guide 2026 | <https://blog.theinterviewguys.com/the-star-method/> |
| The Muse — 30+ Behavioral Interview Questions | <https://www.themuse.com/advice/behavioral-interview-questions-answers-examples> |
| Ophy AI — Technical Interview Prep 8-Week Plan 2026 | <https://ophyai.com/blog/interview-tips/technical-interview-prep-software-engineers> |
| CareerLift AI — How to Prepare for a Software Engineer Interview in 2026 | <https://careerlift.ai/blog/how-to-prepare-software-engineer-interview-2026> |
| Medium (Javarevisited) — Top 8 Resources to Crack System Design and Coding Interviews 2026 | <https://medium.com/javarevisited/top-8-resources-to-crack-the-system-design-and-coding-interviews-in-2026-51d32eac6c07> |
| DigitalDefynd — Top 100 Cyber Security Consultant Interview Questions 2026 | <https://digitaldefynd.com/IQ/top-cyber-security-consultant-interview-questions-and-answers/> |
| Final Round AI — The 25 Most Common Cybersecurity Consultants Interview Questions | <https://www.finalroundai.com/blog/cybersecurity-consultant-interview-questions> |

### 4.5 薪資談判與反問問題

| 來源 | 連結 |
|---|---|
| The Interview Guys — How to Answer Salary Expectations 2026 | <https://blog.theinterviewguys.com/how-to-answer-what-are-your-salary-expectations-in-2026/> |
| Robert Half — How to answer salary expectations | <https://www.roberthalf.com/us/en/insights/career-development/how-to-answer-whats-your-expected-salary> |
| Yourator — 期望薪資怎麼回答 | <https://www.yourator.co/articles/269> |
| Chron (Work) — Great Reverse Interview Questions | <https://work.chron.com/great-reverse-interview-questions-22283.html> |
| Lawyers Mutual NC — 9 Questions to Prepare for a Reverse Job Interview | <https://lawyersmutualnc.com/article/9-questions-to-prepare-for-a-reverse-job-interview/> |
| My Consulting Offer — The 5 Best Questions to Ask in a Consulting Interview | <https://www.myconsultingoffer.org/case-study-interview-prep/questions-to-ask-interviewer/> |
| Hacking the Case Interview — Best Questions to Ask in a Consulting Interview 2026 | <https://www.hackingthecaseinterview.com/pages/questions-to-ask-end-of-consulting-interview> |

### 4.6 跨領域轉職與可轉移技能

| 來源 | 連結 |
|---|---|
| Jobscan — How to Write a Career Change Resume | <https://www.jobscan.co/blog/career-change-resume/> |
| Enhancv — Career Change Resume Tips | <https://enhancv.com/blog/career-change-resume-tips/> |
| Enhancv — Technical Consultant Resume 2026 | <https://enhancv.com/resume-examples/technical-consultant/> |
| Enhancv — Tech Consultant Resume Examples & Guide 2026 | <https://enhancv.com/resume-examples/technology-consultant/> |
| My CV Creator — The Career-Pivot Resume | <https://www.mycvcreator.com/blog/career-pivot-resume> |
| Forbes — The Ultimate Guide To Writing A Career Change Resume（2024） | <https://www.forbes.com/sites/cherylrobinson/2024/05/03/the-ultimate-guide-to-writing-a-career-change-resume/> |
| CompTIA — How To Transition From Software Developer to Cybersecurity | <https://www.comptia.org/en-us/blog/how-to-transition-from-software-development-to-cybersecurity/> |
| Pluralsight — Getting into cybersecurity: transferable skills | <https://www.pluralsight.com/blog/security-professional/getting-into-cybersecurity-transferrable-skills> |
| ExplainThis — 面試如何回答為什麼想換工作 | <https://www.explainthis.io/zh-hant/career/why-quit-current-job> |
| Reeracoen — 過去工作經歷業種不一致能否轉職成功 | <https://www.reeracoen.tw/en/articles/tips100_33> |

### 4.7 職能框架與技術參考（跨領域敘事的證據基礎）

| 來源 | 連結 |
|---|---|
| NIST — Getting Started with the NICE Framework（更新 2025-07-08） | <https://www.nist.gov/itl/applied-cybersecurity/nice/nice-framework-resource-center/getting-started> |
| NIST — Workplace Skills and the NICE Framework（2024-05-16） | <https://www.nist.gov/itl/applied-cybersecurity/nice/nice-framework-resource-center/resources/workplace-skills-and-nice> |
| NIST — Identifying Proficiency within the NICE Framework | <https://www.nist.gov/itl/applied-cybersecurity/nice/nice-framework-resource-center/identifying-proficiency-nice-framework> |
| Google Cloud — MLOps: Continuous delivery and automation pipelines in machine learning | <https://cloud.google.com/architecture/mlops-continuous-delivery-and-automation-pipelines-in-machine-learning> |
| NIST — AI Risk Management Framework（AI RMF 1.0，2023） | <https://www.nist.gov/itl/ai-risk-management-framework> |

!!! warning "引用前請自行查證"
    這份清單是為了讓你知道資訊的出處，方便自行追查原文，**不是本站內容的替代引用來源**。連結會失效、內容會更新，發現不對請自行以標題搜尋確認。

---

## 五、本頁重點回顧

- checklist 分四類：履歷內容、LinkedIn、104／Cake、面試準備，投遞前逐項核對。
- 求職進度追蹤表記錄職缺、投遞日期、階段與後續行動，存在自己的私人檔案裡，不要公開。
- AI 提示詞範本的重點是規格化提問，不是外包判斷。每一段產出都要自己驗證與潤飾。
- 延伸閱讀清單依主題分七類：大學職涯中心、平台官方說明、履歷與 ATS 媒體、面試準備與企業招募頁、薪資談判、跨領域轉職、職能框架與技術參考。
- 所有連結僅供追查原文，不能取代自己對原始來源的查證。

---

**延伸**：[求職起步總覽](getting-started.md)｜[履歷寫作](resume-writing.md)｜[求職平台攻略](platforms.md)｜[面試準備](interview-prep.md)｜[回到首頁](../index.html)
