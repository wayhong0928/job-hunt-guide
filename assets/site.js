...
(function () {
  ...
  function readTheme() {
    try { return localStorage.getItem("jh-theme"); } catch (e) { return null; }
  }
  function writeTheme(v) {
    try { localStorage.setItem("jh-theme", v); } catch (e) { /* 忽略 */ }
  }
  function readVisited(slug) {
    try {
      var raw = localStorage.getItem("jh-visited:" + slug);
  ...
  function writeVisited(slug, value) {
    try { localStorage.setItem("jh-visited:" + slug, String(value)); } catch (e) { /* 忽略 */ }
  }
  ...
  function addUpdateBadge(link) {
    if (link.querySelector(".jh-update-badge")) { return; }
    var badge = document.createElement("span");
    badge.className = "jh-update-badge";
  ...
  function installUpdateStyles() {
    ...
    style.textContent =
      ".jh-update-badge{display:inline-block;margin-inline-start:.45em;padding:.05em .38em;" +
      "border-radius:999px;background:#b54708;color:#fff;font-size:.7em;font-weight:700;" +
      "line-height:1.45;vertical-align:.1em}" +
      ".jh-update-note{margin:.75rem 0 1.25rem;padding:.65rem .85rem;border-inline-start:.25rem solid #b54708;" +
  ...
  document.addEventListener("DOMContentLoaded", function () {
    ...
    if (nav) {
      try {
        var savedScroll = sessionStorage.getItem("jh-sidenav-scroll");
      ...
      nav.querySelectorAll("a").forEach(function (link) {
        link.addEventListener("click", function () {
          try { sessionStorage.setItem("jh-sidenav-scroll", String(nav.scrollTop)); } catch (e) { /* 忽略 */ }
        });
      });
    }
    ...
    var key = "jh-chk:" + location.pathname.split("/").pop();
